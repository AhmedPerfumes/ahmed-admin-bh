...
class OrderController extends Controller
{
    public function storeOrder(Request $request, CreatePaymentForOrderService $createPaymentForOrderService) {
        ...
        if (!$customer_id) {
            $validator = Validator::make($request->all(), [
                ...
                'billingAddress.region'     => 'required|string',
                ]);
            ...
            if (!$exisCustomer) {
                ...
                Address::create([
                    ...
                    'email'     => $request->input('billingAddress.email'),
                    'phone'     => $request->input('billingAddress.mobile'),
                    'state' => $request->input('billingAddress.region'),
                    'city' => $request->input('billingAddress.region'),
                ...
                // curl_setopt($ch, CURLOPT_URL, "https://myinboxmedia.in/api/mim/SendSMS?userid=MIM2300278&pwd=".$password."&mobile=973".$request->input('billingAddress.mobile')."&sender=Ahmedper&msg=".$otp."".urlencode(' is your OTP for Registration')."&msgtype=16");
        ...
        $total = $request->input('finalPrice');
        ...
        if($order) {
            ...
            if($request->input('customer_id')) {
                ...
                OrderAddress::query()->create([
                    ...
                    'state' => $request->input('shippingAddress.region') ? $request->input('shippingAddress.region') : $loggedInCustomerAdd->state,
                    'city' => $request->input('shippingAddress.region') ? $request->input('shippingAddress.region') : $loggedInCustomerAdd->city,
                ...
                OrderAddress::query()->create([
                    ...
                    'state' => $request->input('shippingAddress.region') ? $request->input('shippingAddress.region') : $request->input('billingAddress.region'),
                    'city' => $request->input('shippingAddress.region') ? $request->input('shippingAddress.region') : $request->input('billingAddress.region'),
            ...
            foreach ($request->input('products') as $product) {
                ...
                if(!is_null($exisProduct->discount) && $exisProduct->is_gift != 1 && $exisProduct->is_coupon != 1) {
                    ...
                    if ($product['coupon_type'] == 'percent') {
                        ...
                        $discount_amount = ($product['value'] / (1 + ($tax->percentage / 100))) * $quantity;
                        $net_amount = $total_amount - $discount_amount;
                    }
                    $tax_amount = ($net_amount / 100) * $tax->percentage;
                    $gross_amount = $net_amount + $tax_amount;
                ...
                // elseif(isset($product['is_gift']) && $product['is_gift'] == true) {
                //     $price = $exisProduct->price / (1 + ($tax_amount / 100));
                //     $total_amount = 0.00;
                //     $discount_percent = 100;
                //     $discount_amount = $exisProduct->price / (1 + ($tax_amount / 100));
                //     $net_amount = 0.00;
                //     $tax_amount = 0.00;
                //     $gross_amount = 0.00;
                //     $options = array('name' => $exisProduct->name, 'image' => $exisProduct->image, 'attributes' => ' ', 'taxRate' => $exisProduct->percentage, 'options' => [], 'extras' => [], 'sku' => $exisProduct->sku, 'weight' => $exisProduct->weight, 'original_price' => $exisProduct->price, 'product_type' => $exisProduct->product_type);
            ...
            if($loopSubTotal >= $freeShippingThreshold ?? 400) {
    ...
    public function tapPayment(Request $request, $customerData, $order) {
        $requestParams = [
            ...
            "redirect" => [
                "url" => "http://localhost/ahmed-admin-bh/public/api/tapPaymentRedirect?order_number=".base64_encode($order->code)
            ],
        ];
